...
namespace Etch\Includes\SureCart\Licensing;
...
class Client {
	...
	protected function set_basename_and_slug() {
		...
		$theme_root      = wp_normalize_path( trailingslashit( get_theme_root() ) ); // trailingslashit is important here, without it, theme recognition breaks.

		// it's a plugin.
		if ( strpos( $normalized_file, $theme_root ) === false ) {
			$this->basename = plugin_basename( $this->file ); // No need to normalize the path here because plugin_basename already does it.
			...
			$this->basename = wp_normalize_path( str_replace( $theme_root, '', $normalized_file ) );
